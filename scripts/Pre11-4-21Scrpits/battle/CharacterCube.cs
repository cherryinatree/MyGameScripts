...
public class CharacterCube
{

    public CharacterCube()
    {

    }

    public void FindNeighborCubes(cube cursor_cube, cube.CUBEPHASE changePhaseTo, float distance)
    {/*
    ...
    public void FindSquareNeighborCubes(cube cursor_cube, cube.CUBEPHASE changePhaseTo, float distance)
    {
        CheckCubes(Vector3.forward, cursor_cube, changePhaseTo, distance);
        CheckCubes(-Vector3.forward, cursor_cube, changePhaseTo, distance);
        CheckCubes(Vector3.right, cursor_cube, changePhaseTo, distance);
    ...
    public void CheckCubes(Vector3 direction, cube cursor_cube, cube.CUBEPHASE changePhaseTo, float distance)
    {
    ...
    public void Val(cube start, List<cube> q1, cube.CUBEPHASE changePhaseTo)
    {
    ...
    public void Check(cube start, List<cube> q1, Vector3 direction, cube.CUBEPHASE changePhaseTo)
    {
        Collider[] tempPos1;
        Vector3 halfExtends = new Vector3(0.25f, 0.45f, 0.25f);
            ...
            if (cube != null)
            {
                ...
                foreach (GameObject guy in CharacterMain.theGoodGuys)
                {
                    if (cube != null)
                    {
                        ...
                        if (cube.gameObject == guy.GetComponent<CharacterMain>().my_cube)
                        {
                            cube = null;
                        }
                    }
    ...
    public cube MoveCube(Vector3 direction, cube cursor_cube)
    {
        ...
        foreach (Collider a in tempPosition)
        {
            ...
            {
                cube = a.GetComponent<CharacterMain>().my_cube.GetComponent<cube>();
            }
            if (cube != null)
            {
    ...
    public GameObject MoveGoodGuy(cube cursor_cube)
    {
        ...
        foreach (GameObject a in CharacterMain.theGoodGuys)
        {
            ...
            if (a.GetComponent<CharacterMain>().my_cube == cursor_cube.gameObject
                && a.GetComponent<CharacterMain>().character.myState != Character.BATTLESTATE.TURNOVER)
            {
                PC = a.gameObject;
            }
        }
    ...
    public bool CheckValidMove(cube cursor_cube)
    {
        ...
        if (cursor_cube.myPhase == cube.CUBEPHASE.MOVE)
        {
    ...
    public void ResetVariables()
    {
        ...
        {
            a.GetComponent<cube>().myPhase = cube.CUBEPHASE.NORMAL;
